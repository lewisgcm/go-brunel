...
function calculateTime(stage: Stage) {
	if (stage.StartedAt && stage.StoppedAt) {
		const diff = moment.duration(
			moment(stage.StoppedAt).diff(moment(stage.StartedAt))
		);
...
export const JobProgressGraph = (props: StageGraphProps) => {
	...
	return (
		<svg
			...
			height="110"
			viewBox={`-20 0 ${
				(props.stages.length + 1) * stageSpacing + 40
			} 110`}
		...
		>
			<g>
				...
				{props.stages.map((stage, index) => (
					...
					>
						...
						<text
							...
							y="55"
							textAnchor="middle"
							className={classes.stageDurationText}
						>
							{calculateTime(stage)}
						</text>
						<g
							className={`${classes.stage} ${jobStateClass(
								stage.State,
								stage.ID === props.selectedStageId
							)}`}
						>
				...
				<g
					transform={`translate(${
						(props.stages.length + 1) * stageSpacing
					},50) rotate(0)`}
				>
					<text
						x="0"
