...
export function RepositoryTriggers(props: Props) {
	...
	const [triggers, setTriggers] = useState<RepositoryTriggerItem[]>(
		props.triggers || []
	);
	...
	const isTriggerValid = (trigger: RepositoryTrigger) => {
		return (
			trigger &&
			trigger.Pattern.trim().length > 0 &&
			trigger.Pattern.trim().length < 100
		);
	};

	const onSave = (triggers: RepositoryTrigger[]) => {
		repositoryService.setTriggers(props.id, triggers).subscribe(
			() => {
				setError(undefined);
			},
			() => {
	...
	return (
		<ExpansionPanel className={classes.triggers}>
						...
						{triggers.map((trigger, index) => {
							return (
								<Trigger
									...
									isValid={
										trigger.isValid === undefined
											? true
											: trigger.isValid
									}
									onChange={(
										newTrigger: RepositoryTriggerItem
									) => {
										newTrigger.isValid = isTriggerValid(
											newTrigger
										);
										const copy = triggers.slice();
			...
			<ExpansionPanelActions>
				<Button
					disabled={triggers.some((t) => t.isValid === false)}
					size="small"
					color="primary"
