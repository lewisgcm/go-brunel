...
export class JobService {
	...
	public cancel(id: string): Observable<{}> {
		...
		).pipe(switchMap((response) => response.text() as Promise<{}>));
	}

	public reSchedule(id: string): Observable<Job> {
		return from(
			fetch(`/api/job/${id}/reschedule`, {
				headers: this._authService.getAuthHeaders(),
				method: "POST",
			})
		).pipe(switchMap(handleResponse));
	...
	public containerLogs(containerId: string): Observable<string> {
		...
		).pipe(switchMap((response) => response.text()));
	}
	...
	public progress(id: string): Observable<JobProgress> {
		return timer(0, POLL_INTERVAL_MS).pipe(
			...
			switchMap((since) =>
				from(
					fetch(`/api/job/${id}/progress?since=${since}`, {
						headers: this._authService.getAuthHeaders(),
					})
				).pipe(switchMap(handleResponse))
			),
			scan(
				(accumulated: JobProgress, current: JobProgress) => {
					...
					current.Stages.forEach((stage) => {
						const oldStage = accumulated.Stages.find(
							(s) => s.ID === stage.ID
						);
			...
			takeWhile(
				(progress: JobProgress) =>
					progress.State === JobState.Waiting ||
